...
- `/webhooks/gumroad/refund` — Gumroad refund ping. Same URL-token auth. Looks up `kit_purchases` by `sale_id`, calls `revokeKitAccess()`, sets `refund_status='refunded'`, `invite_status='revoked'`, logs to Google Sheets (when `SHEETS_ENABLED=true`), sends refund-processed email. Idempotent.

### ⚠️ Revoking kit access — always use `revokeKitAccess()`
`DELETE /collaborators/:username` only removes a buyer who **accepted** their invite; a still-pending invitation survives it and stays acceptable. Both endpoints answer 204, so revoking with `removeCollaborator()` alone *looks* successful while leaving a live invite — refund, then accept, keep access forever (found in pre-launch testing, fixed 2026-08-11).
`revokeKitAccess(purchase, env)` does both: `cancelInvitation()` via the stored `github_invite_id` for the pending case, and `removeCollaborator()` for the accepted case. Any new revoke path must call it, and its `SELECT` must fetch `github_invite_id` or the pending branch is silently unreachable.
